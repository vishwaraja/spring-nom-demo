...
function usage {
    echo "usage: insert-reporting-data-without-devices.sh [[-s <docker stack name>] | [-id <subscriber Id>] | [-h]]"
}

stack_name=""
subscriber_id=""
reporting_data="eportal/reporting-data-without-devices.sql"
subscriber_limit=10
...
cat reporting-data-without-devices.sql | sed $replaceString | docker exec -i $vertica_container_name bash -c 'cat > /usr/local/nom/etc/reporting-data.sql'
